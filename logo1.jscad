...
const text = (c, x, y, e, co) => union(vector_text(x, y, c.toString()).map(p => rectangular_extrude(p, {w: 4, h: 4}).scale(e))).setColor(css2rgb(co));

const main = () => {

 const co = [`green`, `red`, `blue`, `yellow`, `black`, `black`];

 const gear = [1, 1.5, 2.5, 4].map(i => involuteGear(Params.Teeth*i, Params.Pitch, Params.pressure, Params.clearance, Params.thickness));
 ...
 return union(color(co[3], gear[0]).rotateZ(18).translate([0.3, 0, 0]), 
   ...
   //                                  cylinder({r: 3, h:10, center: true}).translate([25,0,0]))).translate([20,0,0]));};
              color(co[2], gear[3]).translate([Params.Pitch*2.5, 0, 0]).scale([1, 1, 0.5]));};
